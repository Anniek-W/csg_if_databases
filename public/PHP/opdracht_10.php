...
// Voer een query uit
$sql = "SELECT * FROM artiest,titel,notering WHERE artiest.id=titel.artiest_id AND titel.id=notering.lied_id AND notering.jaar=2014 AND artiest='Nirvana' order by positie ASC";
$records = mysqli_query($DBverbinding, $sql);
      
if (mysqli_num_rows($records) > 0) {
  // Voor elke rij uit de resultaattabel wordt een array aangemaakt
  while($record = mysqli_fetch_assoc($records)) {
    echo "<b>".$record["titel"]."</b>"." uit ".$record["jaar"]." op positie ".$record["positie"].".<br>";
  }
}
else {
  echo "0 resultaten";
